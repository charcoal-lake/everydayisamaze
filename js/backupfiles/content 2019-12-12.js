...
function setup(){
  ...
  // maxIndex = pathLine['H'].length + pathLine['V'].length;
  // console.log(pathLine);

  // generate position for each line
  image(todayMaze, (windowWidth-todayMaze.width)/2, (windowHeight-todayMaze.height)/2);

  /*
  for(var j=0; j<dataSize;j++){
    pool[j]= new TextBox("", 0, 0, false);
  }
  */
  ...
  for (var i=0; i<patch.length; i++){
    ...
    // console.log(x, y);
    // pool[i].posx = (windowWidth-todayMaze.width)/2 + x*cellsize + (cellsize-textsize)/2;
    // pool[i].posy = (windowHeight-todayMaze.height)/2 + y*cellsize +(cellsize-textsize)/2;
    // if(patch[i].dir == 'V') {
    //   pool[i].rotate  = true;
    //   pool[i].canvas.resizeCanvas(textsize, defaultWidth);
    // }

    pool[i] = new TextBox("", posx, posy, patch[i].dir, pathLine[patch[i].dir][patch[i].idx].length);
